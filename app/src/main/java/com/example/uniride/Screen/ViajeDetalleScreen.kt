        ...
        viaje?.let { v ->
            Column(
                ...
                Card(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(16.dp)) {
                    Column(modifier = Modifier.padding(16.dp),
                        ...
                        DetalleRow(Icons.Filled.EventSeat,        "Puestos",     "${v.vehiculo?.capacidad}")
                ...
                if (sesion?.rol != "administrador") {
                    Button(
                        ...
                        enabled = !cargando && v.estado == "disponible",
                        ...
                        if (cargando) CircularProgressIndicator(modifier = Modifier.size(22.dp), strokeWidth = 2.dp)
                            ...
                            Text(if (v.estado == "disponible") "Reservar puesto" else "Viaje lleno",
            ...
            if (showDialog) {
                AlertDialog(
                    ...
                    text = { Text("¿Reservar un puesto?\n\n" +
                            "Origen: ${v.origen}\n" +
                            "Destino: ${v.sede?.nombreSede}\n" +
                            "Costo: $ ${"%.0f".format(v.costo)}") },
                    confirmButton = {
                        Button(onClick = {
                            sesion?.idUsuario?.let { reservaViewModel.reservar(it, v.idViaje) }
                            showDialog = false
                        }) { Text("Confirmar") }
                    },
                    dismissButton = {
